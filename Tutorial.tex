...
\begin{document}
	...
	Ein Inhaltsverzeichnis erstellt man mit \verb|\tableofcontents|. Neben dem Inhaltsverzeichnis gibt es auch das Abbildungsverzeichnis, dieses erstellt man mittels \verb|\listoffigures|, und das Tabellenverzeichnis mit \verb|\listoftables|.
...
\section{Referenzierung}\label{sec:referenzierung}
Um referenzieren zu können, muss zunächst ein Label(\verb|\label(key)|) erstellt werden. Dieses Label kann an die Überschriften, in die Gleichungen, Tabellen oder Abbildungen etc. eingefügt werden.
Die einfachste Form einer Referenzierung kann dann mittels \verb|\ref{label}| erfolgen und sieht so aus \ref{sec:referenzierung}. Dabei ist es notwendig den Typ des Referenzobjekts anzugeben. 
Eine bessere Möglichkeit bietet dann z.B. \verb|\autoref{text}|. Damit können Referenzen in der Form \autoref{sec:referenzierung} erzeugt werden. Es geht aber auch, den Namen der Section zu referenzieren. Dies macht man mit \verb|\nameref{label}| und sieht so aus
\nameref{sec:referenzierung}.

\section{Zitierungen}
Zunächst muss eine Datei *.bib angelegt werden. In dieser Datei werden die Quellen abgelegt.

Damit im Dokument auf diese Quelle verwiesen werden kann muss zunächst die Datei *.bib bekannt gemacht werden. Dies geschieht mit dem kommando \verb|\bibliography(dateiname)|. Daneben muss das Format der Bibliographie festgelegt werden. Dazu gibt es verschiedene Optionen wie z.B. alpha (weitere sind hier zu finden \url{https://en.wikibooks.org/wiki/LaTeX/Bibliography_Management#Bibliography_styles}).

 

Zitieren kann man nun mit dem Kommando \verb|\cite{key}|\cite{davies1992weg}
